...
│   │   └── user_profile.py      # 用户档案管理 API (E-001)
│   ├── core/config.py       # 配置 (Neo4j, LLM, SD 配置)
│   ├── db/seed.py           # 数据库种子数据
│   ├── db/mysql_db.py       # MySQL 数据库模型 (User, PracticeLog, Portfolio)
│   ├── db/migrations/       # 数据库迁移脚本
│   └── services/
│       ├── neo4j_service.py # Neo4j 连接与查询
│       └── llm_service.py   # LLM 集成
├── tests/                   # 测试脚本
│   ├── test_e001_user_models_simple.py  # 模型验证测试
│   ├── test_e001_user_models.py         # 完整集成测试
│   └── test_e001_api.py                 # API 端点测试
└── run.py                   # 入口文件
```
...
| `/user/*` | 用户档案系统 | 用户管理、学习记录、作品集 (E-001) |
...
| GET | `/users/{id}/practice-logs` | 学习记录 |
| GET | `/users/{id}/practice-stats` | 学习统计 |
| GET | `/users/{id}/portfolios` | 作品集 |
| GET | `/users/{id}/profile` | 完整档案 |
...
| 主协调器 (MCP) | 🔴 未开始 | 协议未实现 |
| 用户档案系统 | ✅ 已完成 | E-001: 数据模型、API 端点、测试已完成 |
